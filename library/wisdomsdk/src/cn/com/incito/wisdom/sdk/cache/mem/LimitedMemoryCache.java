...
/**
 * Limited cache. Provides object storing. Size of all stored objects will not to exceed size limit (
 * {@link #getSizeLimit()}).
 * 
 * @see BaseMemoryCache
 */
public abstract class LimitedMemoryCache<K, V> extends BaseMemoryCache<K, V> {
	...
	public LimitedMemoryCache(int sizeLimit) {
		...
		if (sizeLimit > MAX_NORMAL_CACHE_SIZE) {
			WLog.w(LimitedMemoryCache.class, String.format("You set too large memory cache size (more than %1$d Mb)", MAX_NORMAL_CACHE_SIZE_IN_MB));
		}
	}
