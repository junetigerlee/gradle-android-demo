...
import cn.com.incito.wisdom.uicomp.widget.wheelview.ArrayWheelAdapter;
import cn.com.incito.wisdom.uicomp.widget.wheelview.OnWheelChangedListener;
import cn.com.incito.wisdom.uicomp.widget.wheelview.WheelView;

public class CircleCities {
	...
	private int[][] countryArray = { countyOfBeiJing, countyOfTianJing,
			countyOfHeBei, countyOfShanXi1, countyOfNeiMengGu,
			countyOfLiaoNing, countyOfJiLin, countyOfHeiLongJiang,
			countyOfShangHai, countyOfJiangSu, countyOfZheJiang, countyOfAnHui,
			countyOfFuJian, countyOfJiangXi, countyOfShanDong, countyOfHeNan,
			countyOfHuBei, countyOfHuNan, countyOfGuangDong, countyOfGuangXi,
			countyOfHaiNan, countyOfChongQing, countyOfSiChuan,
			countyOfGuiZhou, countyOfYunNan, countyOfXiZang, countyOfShanXi2,
			countyOfGanSu, countyOfQingHai, countyOfNingXia, countyOfXinJiang, };

	private Context mContext;
	private int cityValus = 0;
	private WheelView province, city, country;

	public CircleCities(Context context, View view) {
		...
		province.setAdapter(new ArrayWheelAdapter<String>(provinces));
		...
		country = (WheelView) view.findViewById(R.id.country);
		country.setVisibleItems(7);
		city.addChangingListener(new OnWheelChangedListener() {
			public void onChanged(WheelView wheel, int oldValue, int newValue) {

				String countries[] = mContext.getResources().getStringArray(
						countryArray[cityValus][newValue]);
	...
	public String getPCC() {
		...
		String provinceStr = province.getAdapter().getItem(
				province.getCurrentItem());
		String cityStr = city.getAdapter().getItem(city.getCurrentItem());
		String countryStr = country.getAdapter().getItem(
				country.getCurrentItem());
		return provinceStr + cityStr + countryStr;
	}
}
