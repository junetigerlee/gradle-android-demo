package cn.com.incito.wisdom.uicomp.widget.listview.multicolumn;
...
public class MultiColumnListView extends InternalListView {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs == null) {
            ...
            mHorizontalSpacing = a.getDimensionPixelSize(R.styleable.PinterestLikeAdapterView_plaHorizontalSpacing, 0);
            mVerticalSpacing = a.getDimensionPixelSize(R.styleable.PinterestLikeAdapterView_plaVerticalSpacing, 0);
            a.recycle();
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = (getMeasuredWidth() - mListPadding.left - mListPadding.right - mColumnPaddingLeft - mColumnPaddingRight - mHorizontalSpacing * Math.max(0, mColumnNumber - 1)) / mColumnNumber;

        for (int index = 0; index < mColumnNumber; ++index) {
            mColumns[index].mColumnWidth = width;
            mColumns[index].mColumnLeft = mListPadding.left + mColumnPaddingLeft + (width + mHorizontalSpacing) * index;
        }
