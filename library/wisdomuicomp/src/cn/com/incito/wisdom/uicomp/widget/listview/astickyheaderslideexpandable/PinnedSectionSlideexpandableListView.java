...
import cn.com.incito.wisdom.uicomp.BuildConfig;

/**
 * ListView, which is capable to pin section views at its top while the rest is
 * still scrolled.
 */
public class PinnedSectionSlideexpandableListView extends
        AutoScrollListViewTest {
            ...
            if (isFirstVisibleItemSection) {
                ...
                if (sectionView.getTop() == getPaddingTop()) { // view sticks to
                    ...
                    if (firstVisibleItem == 1) {
                        ensureShadowForPosition(firstVisibleItem - 1,
                ...
                int sectionPosition = findCurrentSectionPosition(firstVisibleItem - 1);
                if (sectionPosition > -1 && firstVisibleItem >= sectionPosition) { // we
                                                                                   // have
                                                                                   // section
                                                                                   // position
    ...
    public PinnedSectionSlideexpandableListView(Context context,
            AttributeSet attrs) {
        super(context, attrs);
        initView();
    }
    ...
    void createPinnedShadow(int position) {
        ...
        Log.i("test", "createPinnedShadow(position)   position = " + position);
        View pinnedView = getAdapter().getView(position, pinnedShadow.view,
                PinnedSectionSlideexpandableListView.this);
    ...
    void ensureShadowForPosition(int sectionPosition, int firstVisibleItem,
        ...
        if (nextPosition < getCount()) {
            ...
            if (nextSectionPosition > -1
                    && (nextSectionPosition - firstVisibleItem) < visibleItemCount) {
    ...
    private PinnedSectionListAdapter getPinnedAdapter() {
        ...
        if (getAdapter() instanceof WrapperListAdapter)
            adapter = (PinnedSectionListAdapter) ((WrapperListAdapter) getAdapter())
                    .getWrappedAdapter();
        else
            adapter = (PinnedSectionListAdapter) getAdapter();
        return adapter;
    }
}