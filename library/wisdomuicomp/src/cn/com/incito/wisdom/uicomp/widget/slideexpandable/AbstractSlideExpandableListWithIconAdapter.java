 ...
 * @date 6/9/12 4:41 PM
 */
public abstract class AbstractSlideExpandableListWithIconAdapter extends
	...
	public AbstractSlideExpandableListWithIconAdapter(ListAdapter wrapped) {
		super(wrapped);
	}
	...
	public abstract ImageView getExpandableIcon(View parent);

	public abstract int getExpandableIconExpand();

	public abstract int getExpandableIconCollapse();
	...
	private void enableFor(final View parent, final View button,
		...
		button.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(final View view) {
				...
				if (listener != null) {
					listener.onClick(parent, view, position);
				}
				...
				if (a != null && a.hasStarted() && !a.hasEnded()) {
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						...
						//
						ImageView icon = getExpandableIcon(parent);
						icon.setImageResource(getExpandableIconExpand());

					} else {
						openItems.set(position, false);
						//
						ImageView icon = getExpandableIcon(parent);
						icon.setImageResource(getExpandableIconCollapse());
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						if (lastOpenPosition != -1
							...
							if (lastOpen != null) {
								...
								animateView(lastParent, lastOpen,
										ExpandCollapseAnimation.COLLAPSE);
							}
					...
					animateView(parent, target, type);
				}
			}
		});
	}
	...
	private void animateView(final View parent, final View target,
			final int type) {
		if (type == ExpandCollapseAnimation.EXPAND) {
			ImageView icon = getExpandableIcon(parent);
			icon.setImageResource(getExpandableIconExpand());
