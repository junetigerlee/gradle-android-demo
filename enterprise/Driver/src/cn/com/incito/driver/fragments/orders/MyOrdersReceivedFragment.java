...
/**
 * 我的订单-待接受
 * @description  我的订单-待接受
 * @author  lizhan
 * @createDate 2014年10月14日
 * @version  1.0
 */
public class MyOrdersReceivedFragment extends
		AbstractMyOrderFragment<ListView, MyOrdersReceivedModel.Response> {

	private static final String[] PROJECTIONS = new String[] {
			AbstractModel.COLUMN_ID, MyOrdersAllModel.COLUMN_ORDERS_ORDER,
			MyOrdersAllModel.COLUMN_ORDERS_GOODS,
			MyOrdersAllModel.COLUMN_ORDERS_CONSIGNE,
			MyOrdersAllModel.COLUMN_ORDERS_SHIPPER,
			MyOrdersAllModel.COLUMN_ORDERS_AGENT,
			AbstractModel.COLUMN_CREATE_TIME };
	...
	protected void loadData(boolean update, boolean loadmore) {
		...
		if (update) {
			...
			String[] selectionArgs = new String[] {
					GlobalModel.getInst().mLoginModel.getCarId(),
					Integer.toString(offset), Integer.toString(mPageSize),
					String.valueOf(Constants.MYORDER_TYPE_APPOINT),
					String.valueOf(fragment.mTimeRange) };
			query(update, loadmore,
					DataProvider.getUri(MyOrdersReceivedModel.class, loadmore),
			...
			query(update, loadmore,
					DataProvider.getUri(MyOrdersReceivedModel.class, loadmore),
					PROJECTIONS, null, null, sortOrder.toString());
		}

	}

	public void onEvent(final MyOrdersReceivedModel.Response resp) {
