...
import cn.com.incito.driver.net.apis.push.RegisterDeviceAPI;
import cn.com.incito.driver.net.apis.push.RegisterDeviceAPI.RegisterDeviceAPIResponse;
 ...
 * 百度推送消息接收器
 * 
 * @description Push消息处理receiver。请编写您需要的回调函数， 一般来说： onBind是必须的，用来处理startWork返回值； onMessage用来接收透传消息；
 *              onSetTags、onDelTags、onListTags是tag相关操作的回调； onNotificationClicked在通知被点击时回调；
 *              onUnbind是stopWork接口的返回值回调 返回值中的errorCode，解释如下： 0 - Success 10001 - Network Problem
 *              30600 - Internal Server Error 30601 - Method Not Allowed 30602 - Request Params Not
 *              Valid 30603 - Authentication Failed 30604 - Quota Use Up Payment Required 30605 -
 *              Data Required Not Found 30606 - Request Time Expires Timeout 30607 - Channel Token
    ...
    public void onBind(Context context, int errorCode, String appid, String userId, String channelId, String requestId) {
        ...
        if (errorCode == 0) {
            ...
            if (NetworkUtils.isNetworkAvaliable(context)) {
                ...
                RegisterDeviceAPI registerDeviceAPI = new RegisterDeviceAPI(channelId, userId);
                new WisdomCityHttpResponseHandler(registerDeviceAPI, new APIFinishCallback() {

                    @Override
                    public void OnRemoteApiFinish(BasicResponse response) {
                        if (response.status == BasicResponse.SUCCESS) {
                            Log.d(TAG, "invoke RegisterDeviceAPI success");

                            RegisterDeviceAPI.RegisterDeviceAPIResponse registerDeviceAPIResponse = (RegisterDeviceAPIResponse) response;
                            Constants.BAIDU_PUSH_USER_TAG = registerDeviceAPIResponse.mTag;

                            Log.d(TAG, "Constants.BAIDU_PUSH_USER_TAG = " + Constants.BAIDU_PUSH_USER_TAG);
                            // Push: 设置tag调用方式
                            List<String> tags = Utils.getTagsList(Constants.BAIDU_PUSH_USER_TAG);
                            PushManager.setTags(mContext, tags);

                        } else {
                            Log.d(TAG, "invoke RegisterDeviceAPI failure");
                        }
                    }
                });

                WisdomCityRestClient.execute(registerDeviceAPI);
            }
    ...
    public void onNotificationClicked(Context context, String title, String description, String customContentString) {
        ...
        if (!TextUtils.isEmpty(customContentString)) {
            ...
            try {
                ...
                // myvalue = customJson.getString("mykey");
                // }
                String panelNo = null;// 01 打开pad可抢货源页面 02 打开pad已抢货源页面
                if (!customJson.isNull("panelNo")) {
                    panelNo = customJson.getString("panelNo");

                    Intent intent = new Intent(context, DriverMainActivity.class);
                    intent.putExtra("panelNo", panelNo);
