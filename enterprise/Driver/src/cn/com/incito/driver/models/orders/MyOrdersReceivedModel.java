...
import cn.com.incito.driver.net.apis.orders.MyReceivedOrdersAPI;
...
/**
 * 我的订单待接受页面的model
 * @description 我的订单待接受页面的model
 * @author  lizhan
 * @createDate 2014年10月16日
 * @version  1.0
 */
public class MyOrdersReceivedModel
        extends
        ListAbstractModel<OrdersReceived, MyReceivedOrdersAPI, MyReceivedOrdersAPI.MyReceivedOrdersAPIResponse> {
    // /logistics/goods/myCollectionGoods
    public static final String sDefaultUrl = "myOrdersReceived";
    ...
    public MyOrdersReceivedModel(Context context, SQLiteDatabase db,
            AbstractDaoMaster daoMaster) {
        super(context, db, daoMaster);
    }

    @Override
    protected Class<OrdersReceived> getDAOModelClassName() {
        return OrdersReceived.class;
    }

    @Override
    protected MyReceivedOrdersAPI newAPIInstance(Map<String, Object> params) {
        return new MyReceivedOrdersAPI(params);
    }

    @Override
    protected List<OrdersReceived> getAPIResponse(
            MyReceivedOrdersAPI.MyReceivedOrdersAPIResponse r) {
        return r.mList;
    }
